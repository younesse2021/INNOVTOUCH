...
namespace XForms.Droid.Renderers
{
    public class CustomPickerRenderer : Xamarin.Forms.Platform.Android.AppCompat.PickerRenderer
    {
        ...
        protected override void OnElementChanged(ElementChangedEventArgs<Picker> e)
        {
            ...
            if (((CustomPicker)this.Element).IsCustom)
            {
                UpdateControl();
            }
            else
            {
                Control.SetBackgroundResource(Resource.Layout.PickerStyle);
            }
        }
        ...
        protected override void OnElementPropertyChanged(object sender, System.ComponentModel.PropertyChangedEventArgs e)
        {
            ...
            //if (e.PropertyName == CustomPicker.IsBorderErrorVisibleProperty.PropertyName)
            if (e.PropertyName == CustomPicker.TextColorProperty.PropertyName ||
                e.PropertyName == CustomPicker.BorderColorProperty.PropertyName ||
                e.PropertyName == CustomPicker.BackgroundColorProperty.PropertyName)
            {
                UpdateControl();
            }
        }
                ...
                if (element.IsCustom)
                {
                    ...
                    if (element.IsHasBorder)
                    {
                        if (element.StyleCorner == ControlStyleCorner.RoundCorner)
                        {
                            ...
                            {
                                shape.SetCornerRadius((float)element.Height / 2);
